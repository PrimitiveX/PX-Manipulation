...
2. Running the evaluation by launching services in both `PX` and `robocasa` environments.  
...
Please download Checkpoint from [🤗 PX-M0-Robocasa](https://huggingface.co/acvlab/PX-M0-Robocasa). You should replace the `base_vlm` in the `config.yaml` file with your own path.
